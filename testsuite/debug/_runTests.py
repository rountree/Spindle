...
# In the original runTests
#
# Most of the tests are specified by "<type>_<mode>".
# The three "serial" tests take the form "serial:<path>".
#
# If TEST_RM was equal to slurm-plugin and SPANK_SPINDLE_USE_SESSION is set, then only session tests are executed.
# If SPINDLE_BLUEGENE is set to true, all "fork" tests are skipped.  (BlueGene is a defunct architecture, there is
#   no need to test for this case.)
#
# As such --run-all-tests will either use the regular tests ( <type> x <mode> ) + ( serial x <paths> ) or the
#   spank-session tests (<type> x session) + ( serial x <paths> ).  Note that there are two additional special
#   cases in spank-sessions tests:  <--start-session> and <--end-session $SESSION_ID>.
#
# Regular tests types and modes are as follows:
regular_test_types = [ 'dependency', 'dlopen', 'dlreopen', 'thrdopen', 'reorder', 'partial', 'ldpreload', 'spindleapi', ]
session_test_types = [ 'session' ]
test_modes = [ 'push', 'pull', 'numa', 'fork', 'forkexec', 'chdir', 'preload', ]
serial_tests = [ 'serial:spindle_exec_test', 'serial:symbind_test', 'serial:interpreter_test' ]

regular_tests = [f"{a}:{b}" for a in regular_test_types for b in test_modes] + serial_tests
session_tests = [f"{a}:{b}" for a in session_test_types for b in test_modes] + serial_tests



ALL_TESTS = [
    ('dependency', 'push'),  ('dependency', 'pull'),  ('dependency', 'numa'),  ('dependency', 'fork'),  ('dependency', 'forkexec'),  ('dependency', 'chdir'),  ('dependency', 'preload'),
    ('dlopen',     'push'),  ('dlopen',     'pull'),  ('dlopen',     'numa'),  ('dlopen',     'fork'),  ('dlopen',     'forkexec'),  ('dlopen',     'chdir'),  ('dlopen',     'preload'),
    ('dlreopen',   'push'),  ('dlreopen',   'pull'),  ('dlreopen',   'numa'),  ('dlreopen',   'fork'),  ('dlreopen',   'forkexec'),  ('dlreopen',   'chdir'),  ('dlreopen',   'preload'),
    ('thrdopen',   'push'),  ('thrdopen',   'pull'),  ('thrdopen',   'numa'),  ('thrdopen',   'fork'),  ('thrdopen',   'forkexec'),  ('thrdopen',   'chdir'),  ('thrdopen',   'preload'),
    ('reorder',    'push'),  ('reorder',    'pull'),  ('reorder',    'numa'),  ('reorder',    'fork'),  ('reorder',    'forkexec'),  ('reorder',    'chdir'),  ('reorder',    'preload'),
    ('partial',    'push'),  ('partial',    'pull'),  ('partial',    'numa'),  ('partial',    'fork'),  ('partial',    'forkexec'),  ('partial',    'chdir'),  ('partial',    'preload'),
    ('ldpreload',  'push'),  ('ldpreload',  'pull'),  ('ldpreload',  'numa'),  ('ldpreload',  'fork'),  ('ldpreload',  'forkexec'),  ('ldpreload',  'chdir'),  ('ldpreload',  'preload'),
    ('spindleapi', 'push'),  ('spindleapi', 'pull'),  ('spindleapi', 'numa'),  ('spindleapi', 'fork'),  ('spindleapi', 'forkexec'),  ('spindleapi', 'chdir'),  ('spindleapi', 'preload'),
]

# Serial-exec tests (use --serial launcher with specific executables)
SERIAL_TESTS = [ './spindle_exec_test', './symbind_test', './interpreter_test', ]

# Session tests - use same test types as ALL_TESTS but with --session flag
SESSION_TEST_TYPES = [ 'dependency', 'dlopen', 'dlreopen', 'thrdopen', 'reorder', 'partial', 'ldpreload', 'spindleapi', ]
...
def parse_arguments():
    ...
    parser.add_argument( '--continue-after-failure',    action='store_true', help='Continue running tests after a failure (default: stop at first failure)')
    parser.add_argument( '--dry-run',                   action='store_true', help='Print commands that would be run without executing them')
    parser.add_argument( '--enable-dmesg-collection',   action='store_true', help='Enable collection of dmesg output (requires sufficient permissions)')
    parser.add_argument( '--flux-interface',            choices=['API', 'CLI'], default='CLI', help='Interface to use with Flux: API (Python API) or CLI (command-line, default). Only valid with --resource-manager=flux.')
    parser.add_argument( '--ignore-test',               metavar='TYPE_MODE', action='append', help='Ignore specific test(s). Same wildcard format as --single-test. ' 'Can be specified multiple times.')
    parser.add_argument( '--list-available-tests',      action='store_true', help='List all available tests and exit')
    parser.add_argument( '--log-dir',                   type=str, default=None, help='Base directory for test logs (default: current directory when --preserve-logs-on-success is used)')
    parser.add_argument( '--num-nodes',                 type=int, default=1, help='Number of nodes to allocate (flux)')
    parser.add_argument( '--preserve-logs-on-success',  action='store_true', help='Keep log directories from successful test runs (default: delete them)')
    parser.add_argument( '--reps',                      type=int, default=1, metavar='N', help='Repeat all requested tests N times (must be positive integer)')
    parser.add_argument( '--resource-manager',          choices=['serial', 'flux'], default='serial', help='Resource manager to use (serial, flux)')
    parser.add_argument( '--run-all-tests',             action='store_true', help='Run all tests appropriate for the selected resource manager')
    parser.add_argument( '--run-serial-tests',          action='store_true', help='Run all serial-exec tests (spindle_exec_test, symbind_test, interpreter_test) - requires serial RM')
    parser.add_argument( '--run-session-tests',         action='store_true', help='Run all session tests (NOT IMPLEMENTED - requires slurm-plugin RM with SPANK)')
    parser.add_argument( '--run-typemode-tests',        action='store_true', help='Run all type_mode tests (56 tests: 8 types × 7 modes)')
    parser.add_argument( '--single-test',               metavar='SPEC', action='append',
    ...
    parser.add_argument( '--spindle-debug',             type=int, choices=[0, 1, 2, 3], help='Set SPINDLE_DEBUG level')
    parser.add_argument( '--tasks-per-node',            type=int, default=1, help='Number of tasks per node (flux)')
    parser.add_argument( '--time-limit',                default='20s', help='Time limit PER TEST, e.g., "5m", "30s" (flux). Accepts Flux duration format.')
    parser.add_argument( '--verbose',                   action='store_true', help='Print command and environment variables. Also prints version at startup.')

    args = parser.parse_args()

    # Print version and all arguments if verbose
    if args.verbose:
        print(f"runTests.py version {__version__}")
        for k, v in args.__dict__.items():
            print(f"{k}: {v}")

    # Handle --list-available-tests (immediate exit)
    if args.list_available_tests: list_available_tests()
    ...
    if args.single_test:
        for test_spec in args.single_test:
            ...
            if len(parts) != 2:
                parser.error(f"--single-test must be in format '<type>_<mode>', '<type>_session', 'serial:<path>', or use wildcards (e.g., '*_push', '*'), got '{test_spec}'")
