# Spindle Flux Podman Container

Rootless Podman-compatible Spindle test container for Flux resource manager.

Includes LLNL-specific workarounds for rootless Podman on LLNL systems.

## Features

- **Rootless compatible**: No capabilities, no sudo
- **Flux RM**: Uses `--with-rm=flux`
...
```bash
cd containers/debug/spindle-flux-podman/scripts
./run_me.sh
```

This will:
1. Build the container image
2. Start a container with Flux broker
3. Run the Spindle test suite (all tests for Flux)
...
```bash
podman run -d \
  --name spindle-flux-test \
  --uidmap 0:0:2000 \
  --uidmap 65534:2000:2 \
  --rm \
  spindle-flux-podman:latest
```

Execute tests:
```bash
podman exec spindle-flux-test bash -c 'cd Spindle-build/testsuite/debug && \
  ./runTests.py --resource-manager=flux --num-nodes=1 --num-tasks=1 --verbose'
```

Stop:
```bash
podman stop spindle-flux-test
```

## Differences from Docker/Flux Versions

- Rootless (no elevated permissions)
- Single node (multi-node requires shared filesystem/networking)
- Simplified Flux configuration
